...
export class GetUserByFilters implements GetUserByFiltersInterface {
  constructor(
    private readonly getUserByFiltersRepository: GetUserByFiltersRepository,
  ) {}

  async execute(
    queryString: GetUserByFiltersInterface.Request,
  ): Promise<GetUserByFiltersInterface.Response> {
    const { email, cpf } = queryString;
    const user = await this.getUserByFiltersRepository.getUserByFilters({
      email,
      cpf,
    });
